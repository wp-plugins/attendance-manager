...
class ATTMGR_User {
	...
	public function is_admin_filter( $result, $user ) {
	...
	public function is_staff_filter( $result, $user ) {
...
//			$userdata = self::get_user_data( $userdata, $user );
	 ...
	 *	Get user data
	 */
	public function get_user_data( $userdata, $user ) {
		$staff_attr = ATTMGR::PLUGIN_ID.'_ex_attr_staff';
...
EOD;
	...
	public function save_extra_fields( $user_id ) {
		...
		if ( isset( $_POST[ $staff_attr ] ) ) {
			update_usermeta( $user_id, $staff_attr, true );
		} else {
			update_usermeta( $user_id, $staff_attr, false );
		}
	}
